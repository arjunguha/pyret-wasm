provide *
# End-to-end SELF-HOSTED **STOPPABLE** compile driver
#
# Chain: surface-parse (pure-Pyret parser, NO JS) -> cps-program (ast->ast CPS
#        stoppability transform) -> desugar-for-anf (cases-based, no visit()) ->
#        fix-provides -> ANF -> wasm-of-pyret backend (which lowers the CPS
#        intrinsics yield-check / finish-result / cps-op-* + exports `resume`).
#
# This is the SINGLE compile path the web IDE uses: source -> CPS -> wasm, parse-once
# (no re-parse), no JS parser, no seed.  It is `compile-driver.arr` with the cps-ast
# transform inserted right after parse, so the emitted module is cooperatively
# interruptible (the Stop button).
#
...
# Re-wrap a parsed program around a new body expression (the CPS'd block), keeping the
# original use/provide/import header. cps-program returns just the top-level Expr.
fun with-body(prog :: A.Program, new-body :: A.Expr) -> A.Program:
  cases(A.Program) prog:
    | s-program(loc, use, prov, prov-types, provides, imports, _) =>
      A.s-program(loc, use, prov, prov-types, provides, imports, new-body)
  end
end
...
fun compile-source(src) -> List<Number>:
  ...
  # CPS stoppability transform (ast -> ast): wrap every fn/loop body in a yield-check
  # interrupt point and thread overloadable ops through cps-op-*; the backend lowers
  # those intrinsics + exports `resume`.  Parse-once: NO render, NO re-parse.
  cps-body = C.cps-program(raw-ast)
  cps-ast = with-body(raw-ast, cps-body)
  desugared = desugar-program(cps-ast)
